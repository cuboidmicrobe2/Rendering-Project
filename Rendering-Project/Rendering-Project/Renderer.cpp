...
void Renderer::Render(Scene& scene, Camera& cam, ID3D11UnorderedAccessView** UAV, RenderingResources* rr) {
    this->ShadowPass(scene.getLights());
    ...
    void Renderer::ShadowPass(const std::vector<Light>& lights, std::vector<SceneObject*> obj) {
        this->GetDeviceContext()->PSSetShader(nullptr, nullptr, 0);

        std::vector<ID3D11DepthStencilView*> dsvs;
        dsvs.reserve(lights.size());
        for (const Light& light : lights) {
            ...
            dsvs.emplace_back(light.GetDepthStencilVeiw());
        }
    ...
    HRESULT Renderer::CreateDeviceAndSwapChain(const Window& window) {
        ...
        return D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, D3D11_CREATE_DEVICE_DEBUG,
                                             nullptr, 0, D3D11_SDK_VERSION, &swapChainDesc,
                                             this->swapChain.GetAddressOf(), this->device.GetAddressOf(), nullptr,
    ...
    HRESULT Renderer::CreateUAV() { // Vertex Shader
        ...
        HRESULT result = this->swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D),
    ...
    void Renderer::RenderParticles(ParticleSystem & particleSystem, Camera & cam) {
        ...
        DirectX::XMStoreFloat4x4(&cameraData.viewProjection, DirectX::XMMatrixMultiplyTranspose(
