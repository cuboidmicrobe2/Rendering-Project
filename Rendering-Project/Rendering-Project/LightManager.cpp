...
HRESULT LightManager::Init(ID3D11Device* device) {
    ...
    r = device->CreateShaderResourceView(this->SpotLightDepthTex.Get(), &srvDesc, this->SpotLightSRV.GetAddressOf());
    if (FAILED(r)) {
    ...
    for (int i = 0; i < this->Spotlights.size(); i++) {
        ...
        HRESULT r =
            this->Spotlights[i].Init(device, this->spotLightViewPort.Width, &dsvDesc, this->SpotLightDepthTex.Get());
        if (FAILED(r)) {
    ...
    r = device->CreateTexture2D(&desc, nullptr, this->dirLightDepthTex.GetAddressOf());
    if (FAILED(r)) {
        std::cerr << "Creating texture Failed, Error: " << r << "\n";
        return r;
    }
    ...
    for (int i = 0; i < this->directionalLights.size(); i++) {
        dsvDesc.Texture2DArray.FirstArraySlice = i;
        HRESULT r =
            this->directionalLights[i].Init(device, this->directionLightViewPort.Width, &dsvDesc, this->dirLightDepthTex.Get());
        if (FAILED(r)) {
            std::cerr << "Creating dsv Failed, Error: " << r << "\n";
            return r;
        }
    }
    data = this->GetDirLightData();
...
ID3D11ShaderResourceView* const* LightManager::GetAdressOfSpotlightDSSRV() const { return this->SpotLightSRV.GetAddressOf(); }
...
ID3D11ShaderResourceView* const* LightManager::GetAdressOfDirlightDSSRV() const { return this->DirLightSRV.GetAddressOf(); }
